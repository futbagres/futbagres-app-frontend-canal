-- Script para recriar as tabelas de sorteio de times
-- Execute este SQL no Supabase Dashboard > SQL Editor

-- Remover tabelas existentes (CASCADE remove políticas também)
DROP TABLE IF EXISTS game_day_team_players CASCADE;
DROP TABLE IF EXISTS game_day_teams CASCADE;

-- Recriar tabela de times
CREATE TABLE game_day_teams (
...
-- Recriar tabela de jogadores dos times
CREATE TABLE game_day_team_players (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Índices
CREATE INDEX idx_game_day_teams_event_date ON game_day_teams(event_id, event_date);
CREATE INDEX idx_game_day_team_players_team ON game_day_team_players(team_id);
